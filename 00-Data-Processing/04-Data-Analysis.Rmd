...
## TEMP: REMOVE INAPPLICABLE DESTINATIONS UNTIL NEW TTMs ARE FINISHED

```{r}
ttm_car <- ttm_car %>%
  filter(!between(OBJECTID, left = 59, right = 69))
...
Half of all parcels can reach 45 foodbanks or more in 180 min or less.
...
min_ttm_transit_pre <- ttm_transit %>%
  #dplyr::filter(COVIDClose == "No") %>%
  group_by(UID) %>%
  summarize(travel_time = min(travel_time),
            linc_age_0to2 = first(linc_age_0to2),
