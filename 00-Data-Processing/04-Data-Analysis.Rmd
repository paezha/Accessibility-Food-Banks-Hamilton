...
Let's do some view maps to see if the results make sense
```{r message = FALSE}
...
tm_shape(access_by_da) + 
  tm_fill("acc_d") +
  tm_shape(foodbank_locations) + 
  tm_dots("COVIDClose")



d_access_by_taz <- accessibility %>%
  st_drop_geometry() %>%
  # Copy the DA identifier to the accessibility table
  left_join(dwelling_network_points_2016 %>%
              st_drop_geometry() %>%
              transmute(UID = ID, TAZUID),
            by = "UID") %>%
  # Group by DA and time
  group_by(TAZUID, timing) %>%
  # Calculate the accessibility by DA before and during covid
  summarize(accessibility = sum(accessibility),
            timing = first(timing),
            .groups = "drop") %>%
...
tm_shape(d_access_by_taz) + 
  tm_fill("acc_d") + 
  tm_shape(foodbank_locations) + 
  tm_dots("COVIDClose")

access_by_taz <- accessibility %>%
  st_drop_geometry() %>%
  # Copy the DA identifier to the accessibility table
  left_join(dwelling_network_points_2016 %>%
              st_drop_geometry() %>%
