...
foodbank_locations <- foodbank_locations %>%
  ...
  filter(st_intersects(x = geometry, y = hamilton_cma, sparse = FALSE))
```
...
Calculate low-income age totals
```{r}
data_da_2016 <- data_da_2016 %>%
  mutate(tot_age_0to2 = `v_CA16_10: Under 1 year` + `v_CA16_13: 1` + `v_CA16_16: 2`,
         linc_age_0to2 = tot_age_0to2 * p_inc_less40k,
         tot_age_0to19 = `v_CA16_7: 0 to 4 years` + `v_CA16_25: 5 to 9 years` + `v_CA16_43: 10 to 14 years` + `v_CA16_64: 15 to 19 years`,
         linc_age_0to19 = tot_age_0to19 * p_inc_less40k,
         tot_age_65plus = `v_CA16_244: 65 years and over`,
         linc_age_65plus = tot_age_65plus * p_inc_less40k,
         linc_population = Population * p_inc_less40k)
```
